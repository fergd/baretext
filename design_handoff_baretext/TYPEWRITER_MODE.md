...
## Theming
All colors come from the active theme's tokens — no hard-coded values. Notably `--syntax-2` differs per theme (Ember `#f8c537`, Parchment `#b8820a`, Amstrad `#7dc45a`, Grove `#e69875`, Dracula `#8be9fd`), so the center guide, marker, and status icon recolor automatically. The edge fade uses `--bg`, so it always matches the current surface.
